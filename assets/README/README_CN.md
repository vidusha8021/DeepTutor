...
> **[2026.6.17]** [v1.4.6](https://github.com/HKUDS/DeepTutor/releases/tag/v1.4.6) — 四个模块全面整合：Space 升级为学习仪表盘，支持导入**我的智能体**并提供顶级 Memory 入口；**知识中心**新增 GraphRAG / PageIndex / LightRAG 引擎，以及关联 KB 和 Obsidian 挂载；Settings 开放文档解析 / 语音 / 图像与视频配置；LLM 能力按分配模型进行门控。

> **[2026.6.14]** [v1.4.5](https://github.com/HKUDS/DeepTutor/releases/tag/v1.4.5) — 引导式学习基于对话智能体循环重建，加入按类型硬门控的掌握度评估与 `/learning` 仪表盘；新增可扩展的循环插件框架，以及 Partner 对话的 Markdown 导出 / 保存至笔记本功能。

> **[2026.6.13]** [v1.4.4](https://github.com/HKUDS/DeepTutor/releases/tag/v1.4.4) — 通过 `deeptutor skill install` 经安全门从 [ClawHub](https://clawhub.ai/) 安装社区技能；知识库文件支持真实的浏览器内 DOCX/XLSX 预览。

> **[2026.6.12]** [v1.4.3](https://github.com/HKUDS/DeepTutor/releases/tag/v1.4.3) — TutorBot 升级为**Partners**，运行在生产级 IM 流水线上（15 个渠道，实时流式传输）；Chat 迁移至单一智能体循环，实现真正的用户级隔离，并重建了 Visualize。

<details>
<summary><b>历史版本（两周前）</b></summary>

> **[2026.5.28]** [v1.4.2](https://github.com/HKUDS/DeepTutor/releases/tag/v1.4.2) — 稳定性与体验优化：解锁 Gemini 2.5+ 在 Visualize 和 Chat 中的使用，修复认证路由问题（#485），平滑流式输出体验，新增最近记录侧边栏，以及 Lemonade 本地提供商支持。

> **[2026.5.27]** [v1.4.1](https://github.com/HKUDS/DeepTutor/releases/tag/v1.4.1) — 安全与稳定性：TutorBot 工具沙箱加固，用户资源隔离，多模态图像回退，TutorBot 的 HTTP/SSE API，以及 v1.4.0 聊天回归修复。

> **[2026.5.22]** [v1.4.0](https://github.com/HKUDS/DeepTutor/releases/tag/v1.4.0) — v1.4 正式版：Auto 模式、三层 Memory、智能体化深度研究 / 解题 / 出题、LlamaIndex RAG 重构、Visualize/Animator 合并，以及重启安全的轮次运行时。

> **[2026.5.21]** [v1.4.0-beta](https://github.com/HKUDS/DeepTutor/releases/tag/v1.4.0-beta) — 三层 Memory 工作台（L1/L2/L3），所有 Chat 能力基于单一智能体引擎重建，仅使用 LlamaIndex RAG，以及统一的 Settings 与能力界面。

> **[2026.5.10]** [v1.3.10](https://github.com/HKUDS/DeepTutor/releases/tag/v1.3.10) — 远程 Docker CORS 修复，SDK 提供商全面支持 `DISABLE_SSL_VERIFY`，更安全的代码块引用，以及可选的 Matrix E2EE 插件。

> **[2026.5.9]** [v1.3.9](https://github.com/HKUDS/DeepTutor/releases/tag/v1.3.9) — TutorBot 支持 Zulip 和 NVIDIA NIM，更安全的思考模型路由，`deeptutor start` 命令，侧边栏工具提示，以及会话存储一致性。

> **[2026.5.8]** [v1.3.8](https://github.com/HKUDS/DeepTutor/releases/tag/v1.3.8) — 可选的多用户部署，支持隔离用户工作区、管理员授权、认证路由和作用域运行时访问。

> **[2026.5.4]** [v1.3.7](https://github.com/HKUDS/DeepTutor/releases/tag/v1.3.7) — 思考模型 / 提供商修复，可见的知识库索引历史，以及更安全的 Co-Writer 清空 / 模板编辑。

> **[2026.5.3]** [v1.3.6](https://github.com/HKUDS/DeepTutor/releases/tag/v1.3.6) — 基于目录的聊天与 TutorBot 模型选择，更安全的 RAG 重新索引，修复 OpenAI Responses token 限制问题，以及技能编辑器校验。

> **[2026.5.2]** [v1.3.5](https://github.com/HKUDS/DeepTutor/releases/tag/v1.3.5) — 更流畅的本地启动配置，更安全的 RAG 查询，本地嵌入认证优化，以及 Settings 深色模式优化。

> **[2026.5.1]** [v1.3.4](https://github.com/HKUDS/DeepTutor/releases/tag/v1.3.4) — Book 页面聊天持久化与重建流程，聊天到 Book 的引用，增强语言 / 推理处理，RAG 文档提取加固。

> **[2026.4.30]** [v1.3.3](https://github.com/HKUDS/DeepTutor/releases/tag/v1.3.3) — NVIDIA NIM + Gemini 嵌入支持，Space 统一聊天历史 / 技能 / 记忆上下文，会话快照，RAG 重新索引韧性。

> **[2026.4.29]** [v1.3.2](https://github.com/HKUDS/DeepTutor/releases/tag/v1.3.2) — 透明的嵌入端点 URL，无效持久化向量的 RAG 重新索引韧性，清理思考模型输出的记忆，修复深度解题运行时问题。

> **[2026.4.28]** [v1.3.1](https://github.com/HKUDS/DeepTutor/releases/tag/v1.3.1) — 稳定性：更安全的 RAG 路由与嵌入校验，Docker 持久化，IME 安全输入，Windows/GBK 健壮性。

> **[2026.4.27]** [v1.3.0](https://github.com/HKUDS/DeepTutor/releases/tag/v1.3.0) — 带重新索引工作流的版本化 KB 索引，重建知识工作区，嵌入自动发现与新适配器，Space 中心。

> **[2026.4.25]** [v1.2.5](https://github.com/HKUDS/DeepTutor/releases/tag/v1.2.5) — 带文件预览抽屉的持久化聊天附件，附件感知能力流水线，TutorBot Markdown 导出。

> **[2026.4.25]** [v1.2.4](https://github.com/HKUDS/DeepTutor/releases/tag/v1.2.4) — 文本 / 代码 / SVG 附件，一键设置引导，Markdown 聊天导出，简洁的 KB 管理界面。

> **[2026.4.24]** [v1.2.3](https://github.com/HKUDS/DeepTutor/releases/tag/v1.2.3) — 文档附件（PDF/DOCX/XLSX/PPTX），推理思考块展示，Soul 模板编辑器，Co-Writer 保存至笔记本。

> **[2026.4.22]** [v1.2.2](https://github.com/HKUDS/DeepTutor/releases/tag/v1.2.2) — 用户自定义技能系统，聊天输入性能全面优化，TutorBot 自动启动，Book 库界面，可视化全屏。

> **[2026.4.21]** [v1.2.1](https://github.com/HKUDS/DeepTutor/releases/tag/v1.2.1) — 分阶段 token 限制，所有入口点的重新生成响应，修复 RAG 与 Gemma 兼容性问题。

> **[2026.4.20]** [v1.2.0](https://github.com/HKUDS/DeepTutor/releases/tag/v1.2.0) — Book 引擎"活书"编译器，多文档 Co-Writer，交互式 HTML 可视化，题库 @-提及。

> **[2026.4.18]** [v1.1.2](https://github.com/HKUDS/DeepTutor/releases/tag/v1.1.2) — 基于 Schema 的 Channels 标签，RAG 单流水线整合，外化聊天提示词。

> **[2026.4.17]** [v1.1.1](https://github.com/HKUDS/DeepTutor/releases/tag/v1.1.1) — 通用"立即回答"，Co-Writer 滚动同步，统一设置面板，流式传输停止按钮。

> **[2026.4.15]** [v1.1.0](https://github.com/HKUDS/DeepTutor/releases/tag/v1.1.0) — LaTeX 块级数学全面优化，LLM 诊断探针，Docker 与本地 LLM 指引。

> **[2026.4.14]** [v1.1.0-beta](https://github.com/HKUDS/DeepTutor/releases/tag/v1.1.0-beta) — 可书签收藏的会话，Snow 主题，WebSocket 心跳与自动重连，嵌入注册表重构。

> **[2026.4.13]** [v1.0.3](https://github.com/HKUDS/DeepTutor/releases/tag/v1.0.3) — 带书签与分类的题本，Visualize 中的 Mermaid，嵌入不匹配检测，Qwen/vLLM 兼容性，LM Studio & llama.cpp 支持，以及 Glass 主题。

> **[2026.4.11]** [v1.0.2](https://github.com/HKUDS/DeepTutor/releases/tag/v1.0.2) — 搜索整合与 SearXNG 回退，提供商切换修复，前端资源泄漏修复。

> **[2026.4.10]** [v1.0.1](https://github.com/HKUDS/DeepTutor/releases/tag/v1.0.1) — Visualize 能力（Chart.js/SVG），测验去重，以及 o4-mini 模型支持。

> **[2026.4.10]** [v1.0.0-beta.4](https://github.com/HKUDS/DeepTutor/releases/tag/v1.0.0-beta.4) — 带限速重试的嵌入进度追踪，跨平台依赖修复，以及 MIME 校验修复。

> **[2026.4.8]** [v1.0.0-beta.3](https://github.com/HKUDS/DeepTutor/releases/tag/v1.0.0-beta.3) — 原生 OpenAI/Anthropic SDK（移除 litellm），Windows Math Animator 支持，健壮的 JSON 解析，以及完整的中文 i18n。

> **[2026.4.7]** [v1.0.0-beta.2](https://github.com/HKUDS/DeepTutor/releases/tag/v1.0.0-beta.2) — 热重载设置，MinerU 嵌套输出，WebSocket 修复，以及 Python 3.11+ 最低版本要求。

> **[2026.4.4]** [v1.0.0-beta.1](https://github.com/HKUDS/DeepTutor/releases/tag/v1.0.0-beta.1) — 智能体原生架构重写（约 20 万行）：工具与能力插件模型，CLI & SDK，TutorBot，Co-Writer，引导式学习，以及持久化记忆。

> **[2026.1.23]** [v0.6.0](https://github.com/HKUDS/DeepTutor/releases/tag/v0.6.0) — 会话持久化，增量文档上传，灵活的 RAG 流水线导入，以及完整的中文本地化。

> **[2026.1.18]** [v0.5.2](https://github.com/HKUDS/DeepTutor/releases/tag/v0.5.2) — RAG-Anything 支持 Docling，日志系统优化，以及 Bug 修复。

> **[2026.1.15]** [v0.5.0](https://github.com/HKUDS/DeepTutor/releases/tag/v0.5.0) — 统一服务配置，按知识库选择 RAG 流水线，出题功能重构，以及侧边栏自定义。

> **[2026.1.9]** [v0.4.0](https://github.com/HKUDS/DeepTutor/releases/tag/v0.4.0) — 多提供商 LLM 与嵌入支持，新主页，RAG 模块解耦，以及环境变量重构。
...
> **[2026.1.2]** [v0.2.0](https://github.com/HKUDS/DeepTutor/releases/tag/v0.2.0) — Docker 部署，Next.js 16 & React 19 升级，WebSocket 安全加固，以及关键漏洞修复。
...
> ⚠️ **必须同时映射 `3782` 和 `8001`。** `3782` 提供 Web UI；`8001` 是 FastAPI 后端，浏览器会直接调用它 — 容器内没有反向代理。跳过 `8001` 映射后页面仍可加载，但 **Settings** 会显示"后端不可达"且无法使用。
...
**远程 Docker / 反向代理：** Web UI 在浏览器中运行，因此浏览器需要能访问后端 URL。对于远程服务器，在 **Settings -> Network** 中配置，或编辑 `data/user/settings/system.json`：
...
`public_api_base` 作为兼容性别名被接受，保存时会规范化为 `next_public_api_base_external`。CORS 使用前端**来源**，而非 API URL。禁用认证时，DeepTutor 默认允许普通 HTTP/HTTPS 浏览器来源。启用认证时，需添加精确的前端来源：
...
**Linux 替代方案 — 宿主机网络：** 添加 `--network=host` 并去掉 `-p` 标志。容器直接共享宿主机网络，打开 [http://127.0.0.1:3782](http://127.0.0.1:3782)（或 `system.json` 中的 `frontend_port`），宿主机服务可通过普通 localhost URL（如 `http://127.0.0.1:11434/v1`）访问。注意宿主机网络会将容器端口直接暴露在宿主机上，可能与现有服务冲突。
...
添加 `--format json` 到任何 `run` 命令，每个轮次将流式输出 **NDJSON — 每行一个事件**（`content`、`tool_call`、`tool_result`、`done` 等），每行带有其 `session_id` 标记。运行是无头安全的：无 TTY 时，`ask_user` 暂停会以空回复自动解决，而不是挂起。
